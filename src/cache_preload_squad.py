"""
Cache Preloader for SQuAD Dataset
==================================

Module ini menyediakan fungsi untuk:
1. Extract dan preload seluruh dokumen context SQuAD ke KV-Cache LLM
...
Workflow:
- Fase 1: Load 200 queries dari SQuAD dev dataset
- Fase 2: Extract semua context paragraphs
...
class SQuADCachePreloader:
    """Preloader untuk SQuAD context documents"""
    ...
    def __init__(
        ...
        self.dataset_path = dataset_path or os.path.abspath(
            '../datasets/squad/dev-v1.1.json'
        )
        self.cache_dir = cache_dir or os.path.abspath('../database/kv_cache/squad')
        ...
        self.context_mapping: Dict[str, str] = {}  # query_id -> context_text

    def load_dataset(self) -> List[Dict]:
        """
        Load SQuAD dataset dan extract 200 queries dengan contexts
        ...
        List[Dict] : List of query dictionaries with fields:
            ...
            - answer: ground truth answer (first answer from possible answers)
        ...
        if not os.path.exists(self.dataset_path):
            raise FileNotFoundError(f"SQuAD dataset not found: {self.dataset_path}")

        with open(self.dataset_path, 'r', encoding='utf-8') as f:
            squad_data = json.load(f)

        # Extract all QA pairs
        # SQuAD structure: data -> articles -> paragraphs -> qas
        all_qa_pairs = []
        for article in squad_data.get('data', []):
            ...
            for paragraph in article.get('paragraphs', []):
                ...
                for qa in paragraph.get('qas', []):
                    ...
                    if question and answers and context:
        ...
        sampled = random.sample(
            all_qa_pairs,
            min(self.num_samples, len(all_qa_pairs))
        )

        # Store queries and extract unique contexts
        self.queries = sampled
        seen_contexts = set()
        unique_contexts = []

        for query in self.queries:
            ctx_text = query['context']
            if ctx_text not in seen_contexts:
                seen_contexts.add(ctx_text)
                unique_contexts.append(ctx_text)
            self.context_mapping[query['id']] = ctx_text

        self.all_contexts = unique_contexts

        print(f"\n✅ Dataset Loaded Successfully")
        print(f"  Total queries: {len(self.queries)}")
        print(f"  Unique contexts: {len(self.all_contexts)}")
        print(f"  Avg queries per context: {len(self.queries) / len(self.all_contexts):.1f}")
        print(f"{'='*70}\n")
    ...
    def build_faiss_vectorstore(
        ...
        for idx, ctx in enumerate(self.all_contexts):
            doc = LCDocument(
                page_content=ctx,
                metadata={'source': 'squad', 'context_id': idx}
            )
...
def quick_test():
    """Quick test function"""
    print("\n" + "="*70)
    print("SQuAD Cache Preloader - Quick Test")
    print("="*70)

    preloader = SQuADCachePreloader(num_samples=10)
